...
namespace RAMMS.Repository.Interfaces
{
    public interface IFormV1Repository : IRepositoryBase<RmFormV1Hdr>
    {
        ...
        #region FormV5
        Task<List<RmFormV5Hdr>> GetFilteredV5RecordList(FilteredPagingDefinition<FormV1SearchGridDTO> filterOptions);

        Task<int> GetFilteredV5RecordCount(FilteredPagingDefinition<FormV1SearchGridDTO> filterOptions);

        Task<List<RmFormV5Dtl>> GetFormv5DtlGridList(FilteredPagingDefinition<FormV5DtlResponseDTO> filterOptions, int V5PkRefNo);

        Task<RmFormV5Hdr> FindFormV5ByID(int id);

        Task<FormV5ResponseDTO> SaveFormV5(FormV5ResponseDTO Formv5);

        Task<int> UpdateFormV5(RmFormV5Hdr FormV5);


        int? SaveFormV5Dtl(RmFormV5Dtl FormV5Dtl);

        Task<int> UpdateFormV5Dtl(RmFormV5Dtl FormV5Dtl);
